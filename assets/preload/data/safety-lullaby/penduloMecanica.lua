...
function win()--I know why I add it here...
	...
	makeAnimatedLuaSprite(shadName, 'Pendelum', 370, 400);
	addAnimationByPrefix(shadName, 'idle', 'Pendelum instance 1', 24, true);
	objectPlayAnimation(shadName, 'idle');
	updateHitbox(shadName);
	scaleObject(shadName, 1.3, 1.3);
	...
	setScrollFactor(shadName,0.9,0.9);
...
function onCreatePost()
	...
	makeAnimatedLuaSprite('pendelum', 'Pendelum', 370, 400);
	...
	setScrollFactor('pendelum',0.9,0.9);
...
function onUpdate()
if getProperty('dad.animation.curAnim.name') == 'idle' and Hypnosis == false then
setProperty('pendelum.offset.x',-13)
setProperty('pendelum.offset.y',-35)
setProperty(shadName..'.offset.x',-13)
setProperty(shadName..'.offset.y',-35)
end

if Hypnosis == true and Nickobelit == true then
       Nickobelit = false
       setProperty('pendelum.offset.x',50)
       setProperty('pendelum.offset.y',-180)
       setProperty(shadName..'.offset.x',50)
       setProperty(shadName..'.offset.y',-180)
       runTimer('adiosOffsetHypnosis', 0.9)
end
end

Hypnosis = false
function opponentNoteHit(id, noteData, noteType, isSustainNote)--Nickobelit lo hizo :0
      if noteData == 0 and Hypnosis == false then
                setProperty('pendelum.offset.x',-25)
                setProperty('pendelum.offset.y',-155)
                setProperty(shadName..'.offset.x',-25)
                setProperty(shadName..'.offset.y',-155)
		end
		if noteData == 1 and Hypnosis == false then
                setProperty('pendelum.offset.x',25)
                setProperty('pendelum.offset.y',-10)
                setProperty(shadName..'.offset.x',25)
                setProperty(shadName..'.offset.y',-10)
		end
		if noteData == 2 and Hypnosis == false then
                setProperty('pendelum.offset.x',0)
                setProperty('pendelum.offset.y',500)
                setProperty(shadName..'.offset.x',0)
                setProperty(shadName..'.offset.y',500)
		end
		if noteData == 3 and Hypnosis == false then
                setProperty('pendelum.offset.x',-170)
                setProperty('pendelum.offset.y',-370)
                setProperty(shadName..'.offset.x',-170)
                setProperty(shadName..'.offset.y',-370)
	end
end
...
function onTimerCompleted(tag, loops, loopsLeft)
    ...
    if tag == 'adiosOffsetHypnosis' then
        Hypnosis = false
end
end
